...
function ActiveRideScreen() {
  ...
  const handleStartRide = () =>
    navigate({ to: "/passageiro/corrida/$rideId/em-andamento", params: { rideId: rideId || "" } });

  return (
    <div className="flex min-h-screen flex-col bg-porcelain font-sans text-navy">
      <header className="p-6 flex items-center justify-between">
        <Button variant="ghost" onClick={() => navigate({ to: "/passageiro/inicio" })}>
          <ArrowLeft />
        </Button>
        ...
        <Button
          variant="ghost"
          onClick={() =>
            navigate({ to: "/passageiro/seguranca", search: { rideId: rideId || "" } })
          }
        >
          <ShieldCheck />
        </Button>
      ...
      <AlertDialog open={isDivergentVehicleAlertOpen} onOpenChange={setIsDivergentVehicleAlertOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Confirme se o veículo ou piloto não conferem.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
