...
import { ChevronLeft, Phone, Chrome, Apple, AlertCircle, CheckCircle2, Loader2, ArrowRight } from "lucide-react";
  ...
  const renderOptions = () => (
    <div className="flex flex-col gap-4 w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <button 
        ...
        className="w-full h-14 bg-navy text-white rounded-2xl font-black uppercase tracking-widest text-[10px] flex items-center px-6 gap-4 hover:bg-navy/90 transition-all active:scale-[0.98] rovya-shadow"
      >
    ...
    <div className="flex min-h-screen flex-col bg-white font-sans text-navy selection:bg-rovya-orange/20">
      <header className="p-8">
        <div className="flex items-center justify-between">
          <button 
            onClick={() => mode === "options" ? navigate({ to: "/passageiro/boas-vindas" }) : setMode("options")}
            className="p-2 -ml-2 hover:bg-slate-50 rounded-xl transition-colors text-slate-400"
          >
