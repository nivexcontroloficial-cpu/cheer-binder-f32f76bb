...
import { History, Clock, Search, CheckCircle2, XCircle, AlertCircle, } from "lucide-react";
import { useDemo } from "@/state/DemoContext";
import { format } from "date-fns";
import { ptBR } from "date-fns/locale";
import { Input } from "@/components/ui/input";
import { rideQuoteSearchSchema, calculateRideFare, getQuoteParams, } from "@/lib/passenger-demo-ride-quote";
...
function RidesHistoryPage() {
    ...
    const { finalFare: demoFinalFare } = useMemo(() => calculateRideFare(searchParams.promoCode), [searchParams.promoCode]);
    ...
    return (<div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20 p-6 bg-porcelain min-h-screen">
      ...
      <div className="space-y-4 mb-6">
        <div className="relative">
          ...
          <Input id="search-rides" type="search" placeholder="Buscar por destino ou ID..." className="pl-10 h-12 bg-white border-slate-100 rounded-2xl text-xs font-medium" value={search} onChange={(e) => setSearch(e.target.value)}/>
        </div>
      ...
      <div className="space-y-3">
        {filteredRides.length > 0 ? (filteredRides.map((ride) => {
            ...
            const CardContent = (<div className="p-4 bg-white border border-slate-100 rounded-[24px] hover:border-navy/10 transition-all">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-2">
                    <div className={`h-8 w-8 rounded-xl flex items-center justify-center ${isCompleted ? "bg-emerald-50 text-emerald-600" : isCancelled ? "bg-rose-50 text-rose-600" : "bg-blue-50 text-blue-600"}`}>
            ...
            if (isCompleted) {
                return (<Link key={ride.id} to="/passageiro/corrida/$rideId/concluida" params={{ rideId: ride.id }} search={(prev) => getQuoteParams(prev)} className="block">
                  {CardContent}
                </Link>);
            }
...
function FilterButton({ active, onClick, label }) {
    return (<button type="button" onClick={onClick} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap min-h-[44px] ${active ? "bg-navy text-white shadow-md" : "bg-white border border-slate-100 text-slate-400 hover:border-slate-200"}`}>
      {label}
    </button>);
}
