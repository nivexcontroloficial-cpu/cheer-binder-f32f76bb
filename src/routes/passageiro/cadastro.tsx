...
import { User, ShieldCheck, Mail, CheckCircle2, ArrowRight, Loader2, Info, ChevronLeft } from "lucide-react";
    ...
    <div className="flex min-h-screen flex-col bg-white font-sans text-navy selection:bg-rovya-orange/20">
      <header className="p-8 pb-4">
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="w-full flex gap-2" role="progressbar" aria-valuenow={step} aria-valuemin={1} aria-valuemax={2}>
            {[1, 2].map((i) => (
              <div 
                key={i} 
        ...
        <form onSubmit={handleNext} className="flex flex-col gap-8 w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="space-y-4">
            <div className="flex items-center gap-2 p-3 bg-rovya-orange/5 border border-rovya-orange/10 rounded-xl">
              ...
              <p className="text-[10px] text-rovya-orange leading-tight font-medium italic">
                Demonstração local: nenhuma conta será criada e os dados informados não serão enviados ou validados.
              </p>
            </div>
          ...
          <div className="space-y-6">
            {step === 1 ? (
              <>
                <div className="space-y-2">
                  <label htmlFor="name-input" className="text-[10px] font-black uppercase tracking-widest text-slate-400">Nome Completo</label>
                  <div className="relative">
                    <User className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={18} strokeWidth={2} aria-hidden="true" />
                    <input 
                      id="name-input"
                      required
                      type="text"
                  ...
                  {errors.name && <p className="text-[10px] text-red-500 font-bold" role="alert">{errors.name}</p>}
                </div>

                <div className="space-y-2">
                  <label htmlFor="email-input" className="text-[10px] font-black uppercase tracking-widest text-slate-400">E-mail</label>
                  <div className="relative">
                    <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={18} strokeWidth={2} aria-hidden="true" />
                    <input 
                      id="email-input"
                      required
                      type="email"
                  ...
                  {errors.email && <p className="text-[10px] text-red-500 font-bold" role="alert">{errors.email}</p>}
                </div>
              </>
            ) : (
              <div className="space-y-6">
                <div className="space-y-2">
                  <label htmlFor="cpf-input" className="text-[10px] font-black uppercase tracking-widest text-slate-400">CPF Fictício (11 números)</label>
                  <div className="relative">
                    <ShieldCheck className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={18} strokeWidth={2} aria-hidden="true" />
                    <input 
                      id="cpf-input"
                      required
                      type="text"
                  ...
                  {errors.cpf && <p className="text-[10px] text-red-500 font-bold" role="alert">{errors.cpf}</p>}
                </div>
                ...
                <div className="flex items-start gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  ...
                  <label htmlFor="terms-checkbox" className="text-[11px] text-slate-500 leading-relaxed font-medium cursor-pointer">
                    Li os{" "}
                    <Link to="/passageiro/termos" className="text-rovya-orange font-bold underline underline-offset-2">
                      Termos de Uso
                    </Link>{" "}
                    e a{" "}
                    <Link to="/passageiro/privacidade" className="text-rovya-orange font-bold underline underline-offset-2">
                      Política de Privacidade
                    </Link>{" "}
                    desta demonstração local.
                  </label>
                </div>
                {errors.terms && <p className="text-[10px] text-red-500 font-bold" role="alert">{errors.terms}</p>}
                ...
                <div className="flex gap-2 p-3 bg-blue-50 border border-blue-100 rounded-xl">
                  ...
                  <p className="text-[9px] text-blue-700 leading-relaxed font-medium italic">
                    Cadastro simulado — sem envio de dados. Seus dados informados permanecem somente nesta tela durante o preenchimento.
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="mt-auto flex flex-col gap-4 pt-4">
            ...
            >
              {isLoading ? <Loader2 size={18} className="animate-spin" /> : step === 1 ? "Continuar" : "Finalizar Simulação"}
      ...
      <footer className="p-8 text-center mt-auto">
        <div className="flex items-center justify-center gap-2 text-[9px] text-slate-300 font-bold uppercase tracking-[0.2em]" aria-hidden="true">
