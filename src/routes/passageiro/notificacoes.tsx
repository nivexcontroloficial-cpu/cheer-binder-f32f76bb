...
const INITIAL_NOTIFICATIONS = [
  {
    ...
    message:
      "Esta é uma notificação simulada de uma viagem que teria ocorrido com um piloto fictício.",
    type: "success",
    isRead: false,
  ...
  },
  {
    ...
    message:
      "Em uma versão real, você receberia avisos sobre novidades e atualizações do sistema aqui.",
    type: "info",
    isRead: false,
    createdAt: new Date().toISOString(),
  },
  {
    ...
    message:
      "Lembre-se: em uma operação real, a conferência de dados do piloto é essencial para sua segurança.",
    type: "warning",
    isRead: true,
        ...
        {notifications.length === 0 && (
          ...
          >
            ...
            <p className="text-xs text-slate-400 mt-1 px-10">
              Não existem notificações na demonstração atual.
            </p>
          </div>
        )}
      </div>
...
function NotificationItem({
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`h-12 w-12 shrink-0 rounded-2xl flex items-center justify-center ${iconColor}`}
        >
        ...
        <div className="flex-1 min-w-0 pr-6">
          <div className="flex items-center gap-2 mb-1">
            ...
            {!notification.isRead && (
              <span
                className="h-2 w-2 rounded-full bg-rovya-orange animate-pulse"
                aria-hidden="true"
              />
            )}
          </div>
