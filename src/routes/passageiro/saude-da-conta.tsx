...
import { ChevronLeft, Shield, HeartPulse, History, ChevronRight, Scale, FileText, Info } from "lucide-react";
    ...
    <div className="max-w-lg mx-auto py-6 px-4 pb-24">
      ...
      >
        ...
        <p className="text-xs text-blue-700 font-medium leading-relaxed">
          Demonstração local: score, ocorrências e protocolos são dados fictícios. Nenhuma penalidade ou decisão real está sendo aplicada à sua conta.
        </p>
      </div>
      ...
      <section className="mb-8">
        <div className="p-6 bg-white border border-slate-100 rounded-[32px] rovya-shadow-sm flex flex-col items-center text-center">
          ...
          <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Status do Score Simulado</h2>
          <div 
          ...
          <p className="mt-3 text-[10px] font-bold text-slate-400">Score exemplificativo: {health.score}/100</p>
        </div>
      </section>

      <section className="space-y-4 mb-8">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-black uppercase tracking-wider text-navy">Protocolos Simulados</h2>
          <Link 
          ...
          {MOCK_PROTOCOLS.map(proto => (
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`h-10 w-10 rounded-xl flex items-center justify-center transition-colors ${proto.status === 'resolvida' ? 'bg-emerald-50 text-emerald-500' : 'bg-blue-50 text-blue-500'}`} aria-hidden="true">
                  <FileText size={20} />
                </div>
                <div>
              ...
              <ChevronRight size={16} className="text-slate-300 group-hover:text-navy transition-colors" aria-hidden="true" />
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="space-y-4 mb-8">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-black uppercase tracking-wider text-navy">Impactos Fictícios no Score</h2>
        ...
        <div className="space-y-3">
          {health.occurrences.map((occ) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`h-10 w-10 rounded-xl flex items-center justify-center transition-colors ${occ.impactScore < 0 ? 'bg-red-50 text-red-500' : 'bg-emerald-50 text-emerald-500'}`} aria-hidden="true">
      ...
      <footer className="mt-8 p-6 bg-slate-900 rounded-[32px] text-white">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="font-black uppercase text-xs tracking-widest">Identificação Demonstrativa</h3>
        </div>
        <p className="text-xs text-slate-300 leading-relaxed">
          Este ambiente é estritamente para demonstração de interface. Os dados de saúde da conta são gerados localmente para ilustrar como o sistema de integridade funcionaria em uma versão futura.
        </p>
      </footer>
    </div>
  );
