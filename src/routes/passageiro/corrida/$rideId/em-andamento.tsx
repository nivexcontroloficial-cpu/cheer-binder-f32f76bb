...
function InProgressRideScreen() {
  ...
  useEffect(() => {
    ...
    }, 1000); // 1% per second approx.

    return () => clearInterval(interval);
  }, [isValidRide, navigate]);
  ...
  return (
    <div className="flex min-h-screen flex-col bg-white font-sans text-navy relative overflow-hidden">
      ...
      <div className="bg-white px-6 pb-[env(safe-area-inset-bottom)] pt-4 space-y-4 shadow-2xl rounded-t-[40px]">
        ...
        {technical && (
          <div className="p-6 bg-slate-900 rounded-[32px] border border-white/10 shadow-xl">
            <h3 className="text-[10px] font-black uppercase text-amber-500 mb-4 flex items-center gap-2">
              <Shield size={12} />
            ...
            <Button
              ...
              className="w-full bg-navy text-white h-11"
            >
