    ...
    <div className="flex flex-col min-h-screen bg-[#F8FAFC] font-sans text-[#111827]">
      <main className="flex-1 flex flex-col items-center justify-center p-6 text-center max-w-5xl mx-auto w-full">
        <div className="mb-12 flex flex-col items-center">
          ...
          <h1 className="text-4xl md:text-5xl font-black tracking-tighter mb-4 uppercase italic text-navy">
            Ambiente Técnico de <span className="text-rovya-orange">Desenvolvimento</span>
          </h1>
          <p className="text-slate-500 max-w-md mx-auto text-[10px] font-bold uppercase tracking-widest">
            Atalhos exclusivos para testes e validação da plataforma
          </p>
        </div>
        ...
        <div className="mt-16 p-6 bg-amber-50 border border-amber-100 rounded-3xl max-w-md">
          ...
          <p className="mt-2 text-xs text-amber-900 leading-relaxed">
            Os dados mostrados são gerados localmente e podem ser resetados. Nenhuma persistência
            real em nuvem está ativa.
          </p>
          <p className="mt-4 text-[10px] text-amber-800 font-bold uppercase tracking-widest leading-relaxed">
            Invariantes de Segurança
          </p>
          <p className="mt-2 text-xs text-amber-900 leading-relaxed">
            Nenhum backend, banco, autenticação real, pagamento real, GPS real ou notificação real
            foi conectado nesta etapa.
          </p>
        </div>
      </main>
...
function DemoCard({
  ...
  return (
    ...
    >
      <div
        className={`h-14 w-14 rounded-2xl flex items-center justify-center mb-6 transition-transform duration-500 group-hover:scale-110 ${bg}`}
      >
        {icon}
      </div>
